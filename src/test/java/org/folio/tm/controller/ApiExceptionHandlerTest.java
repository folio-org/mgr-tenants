...
import org.folio.tm.controller.ApiExceptionHandlerTest.TestController;
...
class ApiExceptionHandlerTest {
  ...
  void handleOutgoingRequestException_positive() throws Exception {
    ...
    when(testService.getTestValue()).thenThrow(new KeycloakException("Failed to perform request", cause));
    mockMvc.perform(get("/tests").queryParam("query", "cql.allRecords=1").contentType(APPLICATION_JSON))
      .andExpect(status().isBadRequest())
      .andExpect(jsonPath("$.total_records", is(1)))
      .andExpect(jsonPath("$.errors[0].message", is("Failed to perform request")))
      .andExpect(jsonPath("$.errors[0].type", is(KeycloakException.class.getSimpleName())))
      .andExpect(jsonPath("$.errors[0].code", is("service_error")))
